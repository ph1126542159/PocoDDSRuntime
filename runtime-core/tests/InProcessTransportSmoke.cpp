...
int main()
{
    ...
    const TopicSpec topic{"orders.created", "example.OrderCreated", "1",
    ...
    auto first = transport.subscribe(topic, [&](const TopicSpec& receivedTopic,
    ...
    if (result.delivered != 1 || result.failed != 1 || observed != 1)
        return 1;
