...
| Cross-host DDS | Two hosts exchange with non-SHM transport and retain trace context | Two isolated Linux network namespaces with distinct interfaces/IPs pass discovery, payload/trace and control; physical two-host evidence pending |
...
| Portability | Windows, Linux and macOS CI plus platform-specific integration evidence | Windows, Linux and macOS core jobs passed; Linux full stack, isolated network stacks and installed SDK consumer passed in GitHub Actions run 29930537604; Windows full stack passed 45/45 |
