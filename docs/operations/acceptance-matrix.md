...
- 受管进程期望状态启用时，在线破坏主快照必须在配置巡检周期内使 ProcessGraph 返回
  `primary-invalid`，`/health/detail` 返回稳定完整性故障码且 `/health/ready` 为 503；不得停止
  仍健康的业务子进程。经授权生命周期提交修复后必须恢复健康，并在 Runtime 重启后保留最终授权。
