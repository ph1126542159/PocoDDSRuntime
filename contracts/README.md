...
- `schemas/team-contract-registry-leader-backend-*.schema.json`：Leader authority 的可插拔外部进程 SPI 与独立 Conformance 证据；固定 config、可执行程序和适配器制品 SHA，通过严格关联的 `read-current`、`read-grant`、`compare-and-swap` 请求访问线性一致存储。超时、进程失败、响应错配、CAS 冲突、历史缺失或任一 pin 漂移都会阻断签发和 Registry 写入；适配器团队可在明确确认的专用空 scope 中验证逐字节读回、不可变历史、陈旧 CAS 和并发唯一胜者，并从安装 SDK 的 `examples/team-contract-registry-leader-backend-file` 复制已实际验收的独立参考实现；默认文件后端仍保持兼容。
- `schemas/team-contract-registry-leader-etcd-*.schema.json`：安装 SDK 的 etcdctl 生产适配路径及只读集群预检证据；至少三个 HTTPS endpoint、etcdctl/CA/客户端证书/私钥和命名空间均被制品摘要固定，预检要求单一 cluster、唯一 member、统一 Leader 与有效 Raft apply 状态，正式 Conformance 再验证同一事务内的 current compare、history create 和 current put。
