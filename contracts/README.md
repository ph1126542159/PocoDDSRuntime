...
- `schemas/team-contract-adapter-certifier-signer-*.schema.json` 与 `tools/team_contract_adapter_certifier_signer.py`：Adapter 认证方的外部 KMS/HSM Signer SPI，固定配置、可执行制品、公钥和 key set，协商 protocol/capability 后只传递目的限定、SHA 绑定的规范 payload；返回签名由主机本地复验。attestation v2 与 Fleet v7 report/journal 只记录 signer/config/capability 摘要，不记录凭证、私钥或本地路径。
- `schemas/team-contract-adapter-certifier-trust-{governance,proposal,approval,activation,state-*}.schema.json`、`schemas/team-contract-adapter-certifier-trust-migration-*.schema.json` 与 `tools/team_contract_adapter_certifier_trust_state_store.py`：以双人 Ed25519 审批、职责分离、不可变 Artifact、线性 CAS/fencing pointer 管理认证信任策略；pointer v2 使用逻辑 Adapter Config 引用。现有 pointer v1 只能经 preflight、固定 proposal、审批和单向 migrate state 原子升级，迁移保持 policy SHA/generation，不接受无证据混合历史或降级。
- `schemas/team-contract-governance-approval-*.schema.json` 与 `tools/team_contract_governance_approval.py`：面向独立团队的通用 payload-bound M-of-N 审批协议，统一 role 阈值/期限、Ed25519 key pin、按时撤销、人员与 key 去重、initiator/approver/executor 职责分离及无路径 evidence；业务模块可通过显式 validator 保留已有 approval envelope。
- `schemas/team-contract-governance-approval-signer-*.schema.json`、`schemas/team-contract-governance-approval-signing-payload.schema.json`、`tools/team_contract_governance_approval_signer.py` 与 `tools/team_contract_governance_approval_signer_admission.py`：Governance Approval 的外部 KMS/HSM/Vault Signer SPI，固定配置、可执行制品和公钥，协商 signer/approver/key/purpose 后签署域分离 payload；创建时准入固定 conformance/attestation/trust provenance 并生成 v3，执行时再以 approval-SHA Readmission Bundle 强制应用当前 Trust Policy、撤销和证据年龄，将无路径 evidence SHA 绑定到三类业务报告。未准入外部 v2 与本地 PEM v1 保持兼容。
- `schemas/team-contract-adapter-conformance-{evidence,attestation,trust-policy}.schema.json` 与 `tools/team_contract_adapter_conformance.py`：统一认证七类公共 Adapter 边界，包括六类 Fleet Adapter 和 Governance Approval Signer；Signer evidence 以 `approverId` 为 scope，独立 Certifier 可对精确 signer kind/ID、config/evidence SHA 签名并由 Trust Policy 执行限权、期限、吊销与 generation 检查。Fleet v6 的六类必需准入集合保持独立。
- `tools/team_contract_adapter_runtime.py`：Backend、Secret Provider、Artifact Store、Config Resolver、Certifier Signer 与 Governance Approval Signer 的公共外部进程宿主 SDK，统一无 shell 执行、制品重校验、环境隔离、超时、响应上限、stderr 脱敏、JSON 边界和稳定能力摘要。各协议仍独立拥有 scope、身份及业务不变量。
