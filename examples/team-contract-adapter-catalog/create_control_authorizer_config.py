#!/usr/bin/env python3
"""Create a digest-pinned Adapter Catalog Control Authorizer config."""
...
def execute(args: argparse.Namespace) -> int:
    try:
        if (IDENTIFIER.fullmatch(args.authorizer_id) is None
            ...
            raise ValueError("Control Authorizer config identity is malformed")
        python = regular(args.python, "Python executable")
        adapter = regular(args.adapter, "Control Authorizer adapter")
        ...
        document = {
            ...
            "authorizerId": args.authorizer_id, "kind": "external-command",
        ...
        print(
            ...
            f"authorizer={args.authorizer_id} "
            f"sha256={hashlib.sha256(content).hexdigest()}"
        )
        return 0
    except (OSError, ValueError) as error:
        print(f"PDR_ADAPTER_CATALOG_CONTROL_AUTHORIZER_CONFIG_ERROR: {error}",
              file=sys.stderr)
        return 2
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--authorizer-id", required=True)
    result.add_argument("--state-environment",
                        default="PDR_CONTROL_AUTHORIZER_STATE_ROOT")
