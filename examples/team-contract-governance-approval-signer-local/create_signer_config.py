#!/usr/bin/env python3
"""Create a pinned local Ed25519 governance approval signer config."""
...
def execute(args: argparse.Namespace) -> int:
    try:
        ...
        if (any(IDENTIFIER.fullmatch(value) is None for value in (
                args.signer_id, args.approver_id))
                or not purposes or not set(purposes).issubset(PURPOSES)
        ...
        mapping, mapping_content = exclusive_json(args.mapping_output, {
            ...
            "signerId": args.signer_id, "approverId": args.approver_id,
            "purposes": purposes, "keys": mappings,
        })
        output, config_content = exclusive_json(args.output, {
            ...
            "requiredCapabilities": CAPABILITIES, "purposes": purposes,
            "keys": keys, "executable": str(executable),
        ...
        print(
            "PDR_APPROVAL_SIGNER_SAMPLE_CONFIG_PASS "
            f"signer={args.signer_id} purposes={len(purposes)} "
            f"keys={len(keys)} "
        ...
        print(f"PDR_APPROVAL_SIGNER_SAMPLE_CONFIG_ERROR: {error}",
              file=sys.stderr)
        return 2
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--approver-id", required=True)
    result.add_argument("--purpose", action="append", required=True)
    result.add_argument(
