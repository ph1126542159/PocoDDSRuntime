#!/usr/bin/env python3
"""Pinned external-command signer for governed approval subjects."""
...
CONFIG_PRODUCT = "PocoDDSRuntimeTeamContractGovernanceApprovalSignerConfig"
...
def validate_config(document: Any) -> None:
    fields = {
        "schemaVersion", "product", "signerId", "approverId", "kind",
        "protocolMajor", "minimumProtocolMinor", "requiredCapabilities",
        "purposes", "keys", "executable", "executableSha256", "arguments",
        "artifactPins", "environmentVariables", "optionalEnvironmentVariables",
        "timeoutSeconds", "maxResponseBytes", "maxPayloadBytes",
    }
    if (not isinstance(document, dict) or set(document) != fields
            ...
            or not set(document["purposes"]).issubset(ALLOWED_PURPOSES)
    ...
    for item in document["keys"]:
        if (not isinstance(item, dict) or set(item) != {
            ...
            raise ValueError("Governance approval signer key is malformed")
        seen.add(item["keyId"])
...
def load_config(path_value: str | Path, expected_sha256: str) \
    ...
    config, path, digest = adapter_runtime.load_pinned_json(
        path_value, expected_sha256, "Governance approval signer",
        validate_config,
    )
    adapter_runtime.revalidate_artifacts(config, "Governance approval signer")
    for item in config["keys"]:
        adapter_runtime.regular_pinned_file(
            item["publicKey"], item["publicKeySha256"],
            "Governance approval signer public key",
        )
...
def validate_capability_manifest(document: Any,
    ...
    if (not isinstance(document, dict) or set(document) != fields
            ...
            or any(document.get(name) != request[name]
                   for name in ("requestId", "signerId", "approverId"))
            or package_tool.IDENTIFIER.fullmatch(str(
        ...
        raise ValueError("Governance approval signer capability manifest is malformed")


def sign_request(config: dict[str, Any], key_id: str, purpose: str,
                 payload: bytes) -> dict[str, Any]:
    return {
        "schemaVersion": 1, "product": REQUEST_PRODUCT,
        "requestId": str(uuid.uuid4()), "signerId": config["signerId"],
        "approverId": config["approverId"], "operation": "sign",
        "purpose": purpose, "keyId": key_id, "algorithm": "Ed25519",
...
def response_signature(document: Any, request: dict[str, Any]) -> bytes:
    fields = {
        "schemaVersion", "product", "requestId", "signerId", "approverId",
        "operation", "purpose", "keyId", "algorithm", "passed",
        "signatureBase64", "errorCode",
    }
    if (not isinstance(document, dict) or set(document) != fields
            or document.get("schemaVersion") != 1
            or document.get("product") != RESPONSE_PRODUCT
            or any(document.get(name) != request[name] for name in (
                "requestId", "signerId", "approverId", "operation", "purpose",
                "keyId", "algorithm",
            )) or type(document.get("passed")) is not bool):
        raise ValueError("Governance approval signer response is malformed")
    if not document["passed"]:
        if (document.get("signatureBase64") is not None
            ...
            raise ValueError("Governance approval signer rejection is malformed")
        raise RuntimeError("Governance approval signer rejected request")
    if document.get("errorCode") is not None:
        raise ValueError("Governance approval signer success is malformed")
    try:
        signature = base64.b64decode(
            document.get("signatureBase64"), validate=True
        )
    except (TypeError, ValueError) as error:
        raise ValueError("Governance approval signer signature encoding is invalid") from error
    if len(signature) != 64:
        raise ValueError("Governance approval signer signature is invalid")
    return signature


class ExternalCommandGovernanceApprovalSigner:
    def __init__(self, config_path: str | Path,
        ...
        adapter_runtime.enforce_capability_policy(
            self.config, manifest, "Governance approval signer"
        )
        if manifest["keyIds"] != sorted(
                item["keyId"] for item in self.config["keys"]):
            raise ValueError("Governance approval signer key identity changed")
        if manifest["purposes"] != self.config["purposes"]:
            raise ValueError("Governance approval signer purpose identity changed")
    ...
    def approver_id(self) -> str:
        return str(self.config["approverId"])
    ...
    def _invoke(self, request: dict[str, Any], *,
        ...
        if config != self.config or path != self.config_path \
            ...
            raise ValueError("Governance approval signer configuration changed")
        ...
        return adapter_runtime.invoke_json(
            ...
            label="Governance approval signer", expose_stderr=False,
        )

    def sign(self, payload: bytes, key_id: str, purpose: str) -> bytes:
        if (not isinstance(payload, bytes)
                or not 1 <= len(payload) <= self.config["maxPayloadBytes"]
                or package_tool.IDENTIFIER.fullmatch(str(key_id)) is None
                or purpose not in self.config["purposes"]):
            raise ValueError("Governance approval signer request is malformed")
        key = next((item for item in self.config["keys"]
                    if item["keyId"] == key_id), None)
        if key is None:
            raise ValueError("Governance approval signer key is not configured")
        request = sign_request(self.config, key_id, purpose, payload)
        ...
        try:
            ...
            raise ValueError(
                "Governance approval signer signature verification failed"
            ) from error
        return signature
...
def main() -> int:
    ...
    if args.self_check:
        print("PDR_GOVERNANCE_APPROVAL_SIGNER_SELF_CHECK_PASS "
